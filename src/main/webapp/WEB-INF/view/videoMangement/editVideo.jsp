...
<body>
	...
	<form class="form-horizontal" action="${pageContext.request.contextPath}/admin/editVideo.action" method="post">
		<div class="form-group">
			<label for="inputEmail3" class="col-md-3 control-label">视频标题</label>
			<div class="col-md-8">
				<input type="text" class="form-control"  name="videoTitle"  value="${vd.videoTitle}"    placeholder="请填写视频标题">
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-md-8">
				<select name="speakerId" style="width: 650px; height: 40px;">  
					...
					<c:forEach var="list" items="${list}" varStatus="status">
						<option value="${list.id}" ${vd.speakerId eq list.id ? "selected" : "" }>${list.speakerName}</option>
					</c:forEach>
				</select>
			</div>
		</div>
			...
			<div class="col-md-8">
				...
				<select name="courseId" style="width: 650px; height: 40px;">
					...
					<c:forEach var="course" items="${course}" varStatus="status">
						<option value="${course.id}" ${vd.courseId eq course.id ? "selected" : ""}>${course.courseName}</option>
					</c:forEach>
				</select>
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-md-8">
				<input type="password" class="form-control"  name="videoLength" value="${vd.videoLength}" placeholder="视频时长">
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="col-md-8">
				<input type="text" class="form-control" name="videoImageUrl" value="${vd.videoImageUrl}"  placeholder="封面图片地址">
			</div>
		</div>
		<div class="form-group">
			<label class="col-md-3 control-label">视频播放地址</label>
			<div class="col-md-8">
				<input type="text" class="form-control" name="videoUrl" value="${vd.videoUrl}" placeholder="封面图片地址">
			</div>
		</div>
		<div class="form-group ">
			...
			<div class="col-md-8 ">
				<textarea class="p100 "  name="videoDescr" 
					 placeholder="请填写视频描述 " style="width: 1000px; height: 100px;">${vd.videoDescr}</textarea>
			</div>
		</div>
